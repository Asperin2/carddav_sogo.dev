        ...
        if ($result = $mysqli->query("SELECT * FROM addressbook ad INNER JOIN user_add_info ui ON ad.uid = ui.uid")) {
            while ($data = $result->fetch_assoc()) {
                try {
                    ...
                    VALUES ('" . $uri . "', '" . $card . "', '" . time() . "', '" . time() . "', '0', " . $inn . ")") or die(mysqli_error($mysqli2));
                    ...
                    VALUES ('" . $uri . "', '" . trim($data['firstname']) . "', '" . trim($data['lastname']) . " " . trim($data['firstname']) . " " . trim($data['firstname2']) . "',
                     '" . trim($data['lastname']) . "', '" . $data['organization'] . "', '" . $data['jobtitle'] . "', 'vcard')") or die(mysqli_error($mysqli2));
                        ...
                        $card = $this->makeVcard($data);
                        if ($this->userActive($data)) {
                            $mysqli2->query("UPDATE " . $config['admin_main_tbl'] . " SET `c_content` = '" . $card . "', `c_lastmodified` = " . time() . ", `c_version` = `c_version` + 1, `c_deleted` = null, `inn` = " . $inn . " WHERE `c_name` = '" . $uri . "'") or die(mysqli_error($mysqli2));
                    ...
                    VALUES ('" . $uri . "', '" . trim($data['firstname']) . "', '" . trim($data['lastname']) . " " . trim($data['firstname']) . " " . trim($data['firstname2']) . "',
                     '" . trim($data['lastname']) . "', '" . trim($data['organization']) . "', '" . $data['jobtitle'] . "', 'vcard')") or die(mysqli_error($mysqli2));
                            ...
                            $mysqli2->query("UPDATE " . $config['admin_main_tbl'] . " SET `c_content` = '" . $card . "', `c_lastmodified` = " . time() . ", `c_version` = `c_version` + 1, `c_deleted` = 1, `inn` = " . $inn . " WHERE `c_name` = '" . $uri . "'") or die(mysqli_error($mysqli2));
                    ...
                    VALUES ('" . $uri . "', '" . trim($data['firstname']) . "', '" . trim($data['lastname']) . " " . trim($data['firstname']) . " " . trim($data['firstname2']) . "',
                     '" . trim($data['lastname']) . "', '" . $data['organization'] . "', '" . $data['jobtitle'] . "', 'vcard')") or die(mysqli_error($mysqli2));
                        ...
                        $card = $this->makeVcard($data);
                        if (!$this->userActive($data)) {
                            $mysqli2->query("UPDATE " . $config['user_main_tbl'] . " SET `c_content` = '" . $card . "', `c_lastmodified` = " . time() . ", `c_version` = `c_version` + 1, `c_deleted` = null, `inn` = " . $inn . " WHERE `c_name` = '" . $uri . "'") or die(mysqli_error($mysqli2));
                    ...
                    VALUES ('" . $uri . "', '" . trim($data['firstname']) . "', '" . trim($data['lastname']) . " " . trim($data['firstname']) . " " . trim($data['firstname2']) . "',
                     '" . trim($data['lastname']) . "', '" . $data['organization'] . "', '" . $data['jobtitle'] . "', 'vcard')") or die(mysqli_error($mysqli2));
                            ...
                            $mysqli2->query("UPDATE " . $config['user_main_tbl'] . " SET `c_content` = '" . $card . "', `c_lastmodified` = " . time() . ", `c_version` = `c_version` + 1, `c_deleted` = 1, `inn` = " . $inn . " WHERE `c_name` = '" . $uri . "'") or die(mysqli_error($mysqli2));
